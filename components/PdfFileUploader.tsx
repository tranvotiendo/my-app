...
export const PdfFileUploader: React.FC<PdfFileUploaderProps> = ({ onFileSelect, file }) => {
      ...
      >
        {file ? (
          ...
          <div className="text-center text-slate-500 dark:text-slate-400">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">{t('pdfToLatex.fileTypes')}</p>
          </div>
        )}
      </label>
      <input id="pdf-file-upload" name="file-upload" type="file" className="sr-only" accept="application/pdf" onChange={handleFileChange} />
    </div>
  );
};